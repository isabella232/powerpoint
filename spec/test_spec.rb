...
describe 'Powerpoint parsing a sample PPTX file' do
  before(:all) do 
    ...
    @deck.add_textual_slide 'Why Android?>', ['Its great&!', 'Its sweet!'] # todo: fix
    ...
    @deck.add_slide(Powerpoint::Slide::DefaultSlide.new(slide_opts))  # todo: fix


    cell = Powerpoint::Slide::TableCell

    top_left = cell.new('top&left', color: 'FFFF00')
    bottom_left = cell.row(%w(bot&left1 bot&left2 bot&left3))
    left_side = cell.weld_vertical(top_left, bottom_left)


    # not welding so we can get a square arrangement here
    right_col = cell.column(%w(top>right1 top&right2 bot&right))
